...
with open("Avaliações/random_forest_evaluation.txt", "w") as f:
    f.write("=== Random Forest - Avaliação ===\n")
    ...
    f.write(f"OOB-score: {rf_model.oob_score_:.4f}\n")
...
plt.tight_layout()
plt.savefig("Avaliações/confusion_matrix_rf.png")
plt.show()

# === Cross-Validation ===
print("\n=== Random Forest - Cross-Validation ===")
with open("Avaliações/random_forest_cross_validation.txt", "w") as f:
    f.write("=== Random Forest - Cross-Validation ===\n")
    for metric in ['precision_weighted', 'recall_weighted', 'f1_weighted']:
        scores = cross_val_score(rf_model, X, y, cv=5, scoring=metric, n_jobs=-1)
...
f1_scores = cross_val_score(rf_model, X, y, cv=10, scoring='f1_weighted')
...
plt.grid(True)
plt.tight_layout()
plt.savefig("Avaliações/f1_score_cross_validation_rf.png")
plt.show()

# Guardar F1-scores per fold
np.savetxt("Avaliações/f1_scores_per_fold_rf.csv", f1_scores, delimiter=",", header="F1-score per fold", comments="")
